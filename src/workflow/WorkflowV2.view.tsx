...
export default function WorkflowViewV2(props: WorkflowProps) {
	...
	const [focusPipeline, setFocusPipeline] = useState<
	...
	useEffect(() => {
		setDrawerHeaderHeight(divRef.current?.clientHeight || 0);
	}, []);
	...
	useEffect(() => {
		...
		if (
			!focusPipeline.isTemplate &&
			!workflowRepo.pipelines.contains(focusPipeline)
		) {
			const samePipelineIndex = workflowRepo.pipelines.findIndex(
				(value, index, object) => value.id === focusPipeline.id
			);
			if (samePipelineIndex === -1) {
				setFocusPipeline(
					getDefaultFocusedPipeline(workflowRepo.pipelines)
				);
			} else {
		...
		if (
			focusPipeline.isTemplate &&
			!workflowRepo.templates.contains(focusPipeline)
		) {
			const samePipelineIndex = workflowRepo.templates.findIndex(
				(value, index, object) => value.id === focusPipeline.id
			);
			if (samePipelineIndex === -1) {
				setFocusPipeline(
					workflowRepo.templates.length > 0
						? workflowRepo.templates[0]
						: undefined
				);
			} else {
	...
	const innerSearchLeafSubjects = (
		rootSubject: SubjectModel,
		leafSubjects: SubjectModel[]
	) => {
	...
	const getFocusPipeline = () => {
		if (!focusPipeline) {
			return (
				...
				>
					<Typography>Find your own way. Trust yourself.</Typography>
					<img
						style={{ marginTop: 5 }}
	...
	const workflowKanban = (
		<WorkflowKanbanViewV2
			kanbanTitle={"workflow"}
		...
		return (
			...
			>
				<div>
					<div
						className={
							currentTab == TabEnums.Workflow
								? "workflow-container-inner-accent-border"
								: "workflow-container-inner"
						}
						style={{ marginTop: 5, fontSize: 16, fontWeight: 600 }}
						onClick={() => setCurrentTab(TabEnums.Workflow)}
					>
						Workflows
					</div>
					<div
						className={
							currentTab == TabEnums.Template
								? "workflow-container-inner-accent-border"
								: "workflow-container-inner"
						}
						style={{ marginTop: 5, fontSize: 16, fontWeight: 600 }}
						onClick={() => setCurrentTab(TabEnums.Template)}
					>
						Templates
					</div>
					<div
						className={
							currentTab == TabEnums.Subject
								? "workflow-container-inner-accent-border"
								: "workflow-container-inner"
						}
						style={{ marginTop: 5, fontSize: 16, fontWeight: 600 }}
						onClick={() => setCurrentTab(TabEnums.Subject)}
					>
						Subjects
					</div>
					<div
						className={
							currentTab == TabEnums.Pomodoro
								? "workflow-container-inner-accent-border"
								: "workflow-container-inner"
						}
						style={{ marginTop: 5, fontSize: 16, fontWeight: 600 }}
						onClick={() => setCurrentTab(TabEnums.Pomodoro)}
					>
						Pomodoro
					</div>
				</div>
	...
	return (
		<div
			style={{
				...
				width: "100%",
				minWidth: 400,
				paddingBottom: drawerHeaderHeight,
			}}
		>
			{getFocusPipeline()}
			{getMenuDrawer()}
